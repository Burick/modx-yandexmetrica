var activeTabs = Ext.decode(MODx.config.yametrica.tab.active);

MODx.panel.YMDashboardWidget = function(config) {
    config = config || {};
    Ext.applyIf(config,{
    cls: 'vertical-tabs-panel'
        ,xtype: 'modx-vtabs'
        ,renderTo: 'ym-panel-widget'
        ,id: 'ym-widget-tabs'
        ,activeTab: 0
        ,border: false
        ,autoWidth: true
        ...
        ,bwrapCfg: {
            tag: 'div'
            ,cls: 'x-tab-panel-bwrap vertical-tabs-bwrap' 
        }
        ,headerCfg: {
            tag: 'div'
        ...
        ,items:[
             {id: 'ym-tab-traffic',     title: _('yametrica.traffic'),      xtype: 'ym-tab-traffic'}
            //,{id: 'ym-tab-sources',     title: _('yametrica.sources'),      xtype: 'ym-tab-sources'}
            //,{id: 'ym-tab-content',     title: _('yametrica.content'),      xtype: 'ym-tab-content'}
            //,{id: 'ym-tab-geography',   title: _('yametrica.geography'),    xtype: 'ym-tab-geography'}
            //,{id: 'ym-tab-demography',  title: _('yametrica.demography'),   xtype: 'ym-tab-demography'}
            //,{id: 'ym-tab-technology',  title: _('yametrica.technology'),   xtype: 'ym-tab-technology'}
            //,{contentEl:'tab3' ,id:'ga-tab-top-content' ,title: _('analytics.top_content')}
        ]
    });
    MODx.panel.YMDashboardWidget.superclass.constructor.call(this, config);
};
Ext.extend(MODx.panel.YMDashboardWidget, Ext.TabPanel);
Ext.reg('ym-panel-dashboard-widget', MODx.panel.YMDashboardWidget);


MODx.panel.YMTabTraffic = function(config) {
    config = config || {};
	var traffic = new Ext.data.JsonStore(
        {
            url: YM.connector_url + '?action=data&data=traffic&format=json',
...
Ext.extend(MODx.panel.YMTabTraffic, Ext.Panel);
Ext.reg('ym-tab-traffic', MODx.panel.YMTabTraffic);


MODx.panel.YMTabSources = function(config) {
	var traffic = new Ext.data.JsonStore({
    ...
    MODx.panel.YMTabSources.superclass.constructor.call(this, config);
};
Ext.extend(MODx.panel.YMTabSources, Ext.Panel);
Ext.reg('ym-tab-sources', MODx.panel.YMTabSources);
...
Ext.onReady(function() {
	var tabs = MODx.load({xtype: 'ym-panel-dashboard-widget'});
    
    for (var index in activeTabs) {
        if (activeTabs[index] == false) {
            Ext.getCmp('ym-widget-tabs').remove('ym-tab-' + index); // TODO: подумать как исправить
        }
    }
});